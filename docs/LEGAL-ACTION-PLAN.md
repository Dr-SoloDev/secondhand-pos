...
**วันที่:** 6 กรกฎาคม 2569 (อัปเดต checklist 26 กันยายน 2569 — ดูแถวที่ระบุวันที่)
...
- **Action 5:** Consent UI สำหรับ PDPA — screen ก่อนบันทึก ID card ทุกครั้ง → **DONE (26 ก.ย. 69)**: บังคับติ๊กตอนสมัคร + บันทึกย้อนหลังได้ + ลายน้ำบนรูป
...
| C5 | ใบรับซื้อมีลายเซ็นผู้ขาย | **PARTIAL** | บังคับเซ็นเฉพาะโลหะมีค่า/ทองแดง (Rule B) — สินค้าทั่วไปยังไม่มี |
| C6 | สามารถ export record ให้เจ้าพนักงานตรวจได้ | PASS (26 ก.ย. 69) | Evidence Pack A4: หัวร้าน+ใบอนุญาต, รายการของ+รูปสี, เลือกเฉพาะบิลคดีได้, ประโยครับรอง+เลขหน้า |
...
| P2 | แจ้ง privacy notice ก่อนเก็บข้อมูล | PASS (26 ก.ย. 69) | ข้อความยินยอมในฟอร์มสมัคร + ลายน้ำบนรูปบัตร |
| P3 | บันทึก consent timestamp | PASS (26 ก.ย. 69) | `pdpa_consented_at`: บันทึกตอนสมัคร + บันทึกย้อนหลังได้ (ปุ่มใน modal / ฟอร์มแก้ไข, stamp ครั้งเดียว) |
| P4 | ส่งข้อมูลผ่าน HTTPS | **FAIL** | ยังไม่มี |
| P5 | ป้องกันการเข้าถึงโดยไม่ได้รับอนุญาต | **FAIL** | admin/admin ยังอยู่ |
| P6 | Data retention policy (เก็บ 5 ปี แล้วลบ) | DEFERRED (26 ก.ย. 69) | เจ้าของสั่งเอาช่องกำหนดลบออกจากจอ + ไม่ทำ purge ในเฟสนี้ — รอเฟสถัดไป |
