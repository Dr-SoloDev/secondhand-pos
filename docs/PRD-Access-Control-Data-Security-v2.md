...
**สถานะ implementation:** ✅ **P0-1..P0-6 IMPLEMENTED ครบ (5 ส.ค. 2569 — branch `feat/p0-access-control`, commits `344075bb` `7d6948c1` `58c491a6`)** — รอ QA ตาม `docs/QA-ACCESS-CONTROL-P0.md` ก่อน merge
...
- [x] เพิ่ม `manager` + **ต้องเพิ่ม branch check คู่กัน** (session/PO ต้องเป็นสาขาเดียวกับผู้ใช้) — pattern อ้างอิงจาก `StockTransfersController::confirm()` (มีอยู่แล้ว)
- [x] manager สาขา A อนุมัติ PO สาขา A → สำเร็จ; PO สาขา B → 403
- [x] log ระบุ role ที่อนุมัติ

#### P0-2: Cashier เพิ่ม/แก้ไขรายการแคทตาล็อก
**สถานะปัจจุบัน:** `createItem()`/`updateItem()` = `['admin','manager']`
- [x] เพิ่ม `cashier` ใน createItem/updateItem
- [x] `deleteItem` **คงจำกัดเฉพาะ `admin`** ตามเดิม
- [x] audit log บันทึก cashier คนไหนแก้รายการอะไร เมื่อไหร่ (มีอยู่แล้ว — verify ครบ)
...
- [x] `cashier` เปลี่ยน `tier_level` (1-3) ได้อิสระ **ไม่มี guardrail** — cashier คือคนจัดการข้อมูลนี้ตลอดเวลา
- [x] audit log `update_seller_tier` ระบุ cashier คนไหนเปลี่ยน

#### P0-4: Cashier ออกใบโอนสต็อก + ยืนยันรับสต็อกสาขาตัวเอง
**สถานะปัจจุบัน:** `store()`/`confirm()` = `['admin','manager','super_manager']`
- [x] เพิ่ม `cashier` ใน `store()` และ `confirm()`
- [x] ✅ **Verified: branch check มีอยู่แล้วทั้ง 2 ฟังก์ชัน** (confirm: to_branch ต้องตรง branch ผู้ใช้ → 403; store: ห้ามโอนจากสาขาอื่น → 403) — แค่เพิ่ม role ใน requireAuth list
- [x] `cancel()/requestReversal()/approveReversal()/rejectReversal()` คงเดิม (admin/manager/super_manager)
...
- [x] เพิ่ม `manager` ใน approve/reject ของ cash session
- [x] **ต้องเพิ่ม branch check คู่กัน** (cash session ที่จะอนุมัติ ต้องเป็นสาขาเดียวกับ manager) — ตรวจว่า controller มี branch check เดิมหรือไม่ ถ้าไม่มีให้เพิ่ม (pattern เดียวกับ P0-1)
- [x] cashier สาขา A เปิดยอด → manager สาขา A อนุมัติได้ / manager สาขา B อนุมัติไม่ได้ (403)
...
- [x] แก้ `common.js` ให้ role matrix ตรงกับ backend: cashier/manager เห็นเมนูที่ API อนุญาต (เปิด-ปิดยอด, แคทตาล็อก, โอนสต็อก ตาม scope)
- [x] super_manager ยังคงเห็นเฉพาะที่กำหนดเดิม (financial-summary) + เปิด-ปิดยอดตามที่ API อนุญาต
- [x] admin เห็นทั้งหมดตามเดิม
- [x] **ไม่ redesign** — แค่ visibility ตาม role
...
### Implementation Status — P0-1..P0-6 (5 ส.ค. 2569, branch `feat/p0-access-control`)

| P0 | สรุปการ implement | commit |
|----|-------------------|--------|
| P0-1 | `PurchaseOrdersController::approveCancellation()/rejectCancellation()` เพิ่ม `manager` + branch check ผ่าน `PurchaseOrderCancellation::getRequestBranch()` (สาขาไม่ตรง → 403) | `7d6948c1` |
| P0-2 | `PurchaseItemCatalogController::createItem()/updateItem()` เพิ่ม `cashier`; `deleteItem` คง admin-only | `344075bb` |
| P0-3 | `SellersController` tier create/update เพิ่ม `cashier` (ไม่มี guardrail ตาม Owner decision) | `344075bb` |
| P0-4 | `StockTransfersController::store()/confirm()` เพิ่ม `cashier` (branch check มีอยู่เดิม) | `344075bb` |
| P0-5 | `CashSessionsController::approveOpen()/rejectOpen()/approveClose()/rejectClose()` เพิ่ม `manager` + branch check ผ่าน `CashSession::getBranch()` | `7d6948c1` |
| P0-6 | `common.js` เขียน `applyRoleNavigation()` ใหม่เป็น data-driven `PAGE_ROLE_ACCESS` ตรงกับ backend matrix | `58c491a6` |

Test: `tests/api/test_role_permissions.sh` — ครอบ P0-2/P0-3/P0-4 cashier + P0-1/P0-5 branch-mismatch 403 (commit `7d6948c1`). QA ขั้นตอน: `docs/QA-ACCESS-CONTROL-P0.md`
...
| ช่วง | งาน | หมายเหตุ |
|---|---|---|
| **ตอนนี้ → Deploy** | P0-1 ถึง P0-6 ทั้งหมด | ลำดับ: P0-4 → P0-2 → P0-3 → P0-1 → P0-5 → P0-6; test หลังทุกจุด; UI งานสุดท้าย (ไม่บล็อก backend) — **✅ implement ครบ 5 ส.ค. 2569** |
