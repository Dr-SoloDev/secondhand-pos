...
ALTER TABLE purchase_order_items
  ADD COLUMN IF NOT EXISTS weight_source ENUM('manual','scale','manual_override') NOT NULL DEFAULT 'manual'
  ...
  ADD COLUMN IF NOT EXISTS scale_device_id INT NULL
    COMMENT 'scale_devices.id — null ถ้า manual' AFTER weight_source,
  ADD COLUMN IF NOT EXISTS scale_raw_kg DECIMAL(10,2) NULL
    COMMENT 'ค่าน้ำหนักดิบจากตาชั่งก่อนคำนวณ' AFTER scale_device_id,
  ADD COLUMN IF NOT EXISTS scale_stable TINYINT(1) NULL
  ...
  ADD INDEX IF NOT EXISTS idx_poi_weight_source (weight_source),
  ADD INDEX IF NOT EXISTS idx_poi_scale_device (scale_device_id);
